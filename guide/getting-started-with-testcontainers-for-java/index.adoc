...
We are going to create *CustomerService* class to manage user details.
...
* We have the *createCustomersTableIfNotExists()* method that creates the users table if it does not already exist.
* We have *createCustomer()* method that inserts a new user record into the database.
* We have the *getAllCustomers()* method that fetches all rows from *users* table, populates data into *Customer* objects
...
* In *@BeforeEach* callback method, which gets executed before running every test method, we have created a *DBConnectionProvider* instance
  ...
  In the *CustomerService* constructor, we are creating the users table if it does not already exist.
* We have a *shouldGetCustomers()* test where we are inserting 2 user records into the database, fetching all the existing users
  and asserting the number of users.
